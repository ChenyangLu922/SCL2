    ...
    def __init__(self, dataset, r, noise_mode, batch_size, num_workers, root_dir, noise_file=''):
        ...
        if self.dataset == 'cifar10':
            self.transform_train = transforms.Compose([
                #transforms.RandomResizedCrop(size=32, scale=(0.2,1.)),#new
                transforms.RandomCrop(32, padding=4),
                transforms.RandomHorizontalFlip(),
                transforms.RandomApply([
                    transforms.ColorJitter(0.4,0.4,0.4,0.1)
                ],p=0.8),#new
                transforms.RandomGrayscale(p=0.2),#new
                transforms.ToTensor(),
    ...
    def run(self, mode):
        if mode == 'train':
            train_dataset = cifar_dataset(dataset=self.dataset, noise_mode=self.noise_mode, r=self.r,
                                        root_dir=self.root_dir, transform=TwoCropTransform(self.transform_train), mode="train",
                                        noise_file=self.noise_file)#add two crop transform
            trainloader = DataLoader(
                dataset=train_dataset,
                batch_size=self.batch_size,
                shuffle=True,
                num_workers=self.num_workers)
            return trainloader,np.asarray(train_dataset.noise_label),np.asarray(train_dataset.clean_label),train_dataset
            ...
            eval_loader = DataLoader(
                ...
                shuffle=False,
                num_workers=self.num_workers)
            return eval_loader,np.asarray(eval_dataset.noise_label),np.asarray(eval_dataset.clean_label),eval_dataset
